...
class Product extends MysqlBaseModel
{
    ...
    public function product_brand($id = null)
    {
        if ($id != null)
            ...
            ON brand.`id` = pro.`brand_id`
			LEFT JOIN `photos` as photo
            ON photo.`entity_type` = 'Brand'
            AND photo.`entity_id` = brand.`id`
			WHERE pro.`id` = $id
    ...
    public function join_product_to_photo__for_latest_product()
    {
        ...
        FROM `products` as pro
        INNER JOIN `photos` as img
		ON pro.`id` = img.`entity_id`
        WHERE img.`entity_type` = 'Product'
        AND img.`type` = 0
    ...
    public function join_product_to_photo_by_brand_id($brand_id)
    {
        ...
        FROM `products` as pro
        INNER JOIN `photos` as img
        ON img.`entity_id` = pro.`id`
        AND img.`entity_type` = 'Product'
        AND img.`type` = 0
    ...
    public function join_product_to_photo_all()
    {
        ...
        $queryStr = "
            SELECT
            pro.*,
            img.`path` as photos_path,
            img.`alt` as photos_alt,
            pro.`id` as product_id,
            dis.`title` AS discounts_title,
            dis.`percent` AS discounts_percent,
            dis.`id` AS discounts_id,
            dis.`status` AS discounts_status
			FROM
            `products` as pro
            INNER JOIN `photos` as img
			ON pro.`id` = img.`entity_id`
    ...
    public function join_product__with_comment_and_like($id)
    {
        return $this->query("
        SELECT *
        FROM `products` as `pros`
        INNER JOIN `comments` as `comments`
        ON `comments`.`entity_id` = pros.`id`
        WHERE `comments`.`entity_type` = 'Product'
    ...
    {
        return $this->query("
        SELECT pros.*,img.`path`,img.`alt`
        FROM `products` as `pros`
        INNER JOIN `photos` as `img`
        ON `img`.`entity_id` = pros.`id`
        AND `img`.`entity_type` = 'Product'
        AND `img`.`type` = 0 
        WHERE pros.brand_id=$brand_id");
    }
